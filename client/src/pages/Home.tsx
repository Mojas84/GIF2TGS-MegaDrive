...
import { Activity, CheckCircle2, Download, Gamepad2, Play, RotateCcw, UploadCloud, X, XCircle } from "lucide-react";
...
export default function Home() {
  ...
  return (
    <div className="console-shell"><div className="scanlines" aria-hidden="true" />
      <header className="topbar"><div className="brand-lockup"><div className="brand-icon"><Gamepad2 size={18} strokeWidth={2.5} /></div><div><div className="brand-name">TGS_CONSOLE</div><div className="brand-edition">SEGA MODE / 199X EDITION</div></div></div><div className="system-status"><span className="status-dot" /> SYSTEM ONLINE</div></header>
      <main className="hero-grid"><section className="hero-copy"><div className="eyebrow"><span /> MEGA DRIVE UTILITY</div><h1><span className="acid">GIF</span><span className="arrow">→</span><span className="coral">TGS</span></h1><h2>MEGA DRIVE CONVERTER</h2><p className="hero-description">Nahraj animaci, sleduj její převod a získej Telegram sticker s kontrolou limitů v reálném čase. Bez účtu. Bez cloudu. Bez magie.</p><div className="limit-grid"><div className="limit-card"><span>01</span><strong>64 KB</strong><small>TGS MAX</small></div><div className="limit-card"><span>02</span><strong>3 SEC</strong><small>LOOP MAX</small></div><div className="limit-card"><span>03</span><strong>512 PX</strong><small>CANVAS</small></div></div></section>
        <section className="converter-wrap"><img className="mascot" src="/mascot.png" alt="" aria-hidden="true" /><div className="converter-card"><div className="section-kicker"><span>[ 01 / LOAD GIF ]</span><b>2</b></div><h3>INSERT ANIMATION</h3>
          <label className={`drop-zone ${state === "error" ? "drop-error" : ""} ${file ? "has-file" : ""}`} onDragOver={event => event.preventDefault()} onDrop={onDrop}><input ref={inputRef} className="file-input" aria-label="Choose GIF file" type="file" accept="image/gif,.gif" onChange={onInputChange} />
            {file ? <div className="gif-preview"><img src={fileUrl} alt="Náhled GIFu" /><div className="file-line"><div><strong>{file.name}</strong><span>{formatBytes(file.size)} // INPUT GIF // MAX 8 MB</span></div><button type="button" onClick={event => { event.preventDefault(); reset(); }} aria-label="Odebrat GIF"><X size={15} /></button></div></div> : <><div className="upload-square"><UploadCloud size={24} /></div><strong>DROP GIF HERE</strong><span className="drop-caption">GIF ONLY // MAX INPUT 8 MB</span><span className="drop-hint">CLICK TO SELECT OR DRAG FILE</span></>}
          </label><div className="action-row"><button className="convert-button" disabled={!isReady} onClick={convert}><span className="button-index">3</span><Play size={13} fill="currentColor" /> {state === "converting" ? "PROCESSING..." : "START CONVERSION"}</button><button className="reset-button" onClick={reset} aria-label="Reset converter"><RotateCcw size={13} /> RESET</button></div><div className={`progress-console state-${state}`} aria-live="polite"><Activity size={13} /><span>{message || "SEGA MODE // PIXELART2TGS ENGINE // TEMP FILES PURGED"}</span></div>{state === "converting" && <div className="progress-bar"><span /></div>}{state === "error" && message && <div className="error-panel"><XCircle size={16} /><span>{message}</span></div>}
        </div>
