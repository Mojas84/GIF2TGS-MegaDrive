...
const MAX_TGS_RAW_BYTES = 1_000_000;

type TgsMeta = { size: number; rawSize: number | null; frames: number; fps: number; duration: number; width: number; height: number };
...
export default function Home() {
  ...
  const convert = async () => {
    ...
    try {
      ...
      const rawSizeHeader = response.headers.get("X-TGS-Raw-Size");
      setMeta({ size: blob.size, rawSize: rawSizeHeader ? Number(rawSizeHeader) : null, fps, frames, duration: Number(response.headers.get("X-TGS-Duration") || (frames / fps).toFixed(2)), width: Number(response.headers.get("X-TGS-Width") || 512), height: Number(response.headers.get("X-TGS-Height") || 512) });
  ...
  const readyChecks = meta && meta.size <= 64 * 1024 && meta.rawSize !== null && meta.rawSize <= MAX_TGS_RAW_BYTES && meta.duration <= 3 && meta.width === 512 && meta.height === 512;
    ...
    <main className="retro-scanlines">
      <div className="app-frame">
        ...
        <section className="hero-grid">
          ...
          {state === "success" && outputUrl && meta && <div className="results-grid"><section className="output-card"><div className="result-kicker">[ 02 / TGS OUTPUT ]</div><h3>PREVIEW PLAYER</h3><div className="tgs-preview-frame"><TgsPreview url={outputUrl} /><span>TGS DATA // GZIP LOTTIE PLAYER</span></div><div className="output-footer"><div><strong>{outputName}</strong><small>{formatBytes(meta.size)} // GZIP TGS</small></div><a className="download-button" href={outputUrl} download={outputName} onClick={() => playSfx("confirm")}><Download size={14} /> SAVE</a></div></section><aside className="diagnostics-card"><div className="result-kicker">[ 03 / DIAGNOSTICS ]</div><h3>TELEGRAM LIMIT CHECK</h3><div className={`telegram-status ${readyChecks ? "ready" : "warn"}`}>STATUS: {readyChecks ? "TELEGRAM READY" : "OPTIMIZATION ADVISED"}</div><div className="checks"><CheckRow ok={meta.size <= 64 * 1024} label="Komprimovaná velikost" value={formatBytes(meta.size)} hint="LIMIT ≤ 64 KB" /><CheckRow ok={meta.rawSize !== null && meta.rawSize <= MAX_TGS_RAW_BYTES} label="Rozbalený Lottie JSON" value={meta.rawSize === null ? "neověřeno" : formatBytes(meta.rawSize)} hint="KOMPATIBILITA ≤ 1 000 000 B" /><CheckRow ok={meta.duration <= 3} label="Délka animace" value={`${meta.duration.toFixed(2)} s`} hint="LIMIT ≤ 3 s" /><CheckRow ok={meta.width === 512 && meta.height === 512} label="Plátno" value={`${meta.width} × ${meta.height}`} hint="LIMIT 512 × 512 px" /></div><div className="stat-row"><span><b>{meta.frames}</b>frames</span><span><b>{meta.fps}</b>fps</span><span><b>{meta.duration.toFixed(2)}s</b>duration</span></div></aside></div>}
          </div>
        </section>
